...
documentada, não o padrão a copiar. A razão prática: editar a cópia versionada aqui
não conserta a fonte em `ai-lup-skills`, e uma correção feita lá não chega aqui
sozinha.

Se você estiver trabalhando **na fonte** de uma dessas skills, o caminho é o catálogo
`ai-lup-skills`, não os arquivos dentro de `.claude/skills/` deste repositório — editar
aqui é editar uma cópia. O `blueprintfy` é a única indispensável em tempo de execução:
`context_config.py` importa dela o parser de front matter, e `ask`/`validate.py`
importam o `repo_cache.py`. Sem ela, os scripts param com uma mensagem dizendo isso.
